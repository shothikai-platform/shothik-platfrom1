...
import CitationAnalysisPanel from "@/components/plagiarism/CitationAnalysisPanel";
...
const PlagiarismCheckerContentSection = () => {
  ...
  return (
    <div className={cn(
      ...
      {isMobile && (
        <div className="mb-3 flex w-full rounded-xl bg-muted/50 p-1 shadow-sm" role="tablist" aria-label="Plagiarism checker views">
          <button
            id="editor-tab"
            role="tab"
      ...
      <div className={cn(
              ...
              <div className={cn(
                ...
                {hasReport && highlightRanges.length > 0 && (
                  <>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-muted-foreground text-xs">Paraphrased</span>
                    </div>
                  </>
                )}
                {excludeLatex && stemDetection.hasLatex && (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-muted-foreground text-xs">LaTeX (excluded)</span>
                  </div>
                )}
                {excludeCode && stemDetection.hasCode && (
                  <div className="flex items-center gap-1.5">
                    <span className="inline-block h-2.5 w-2.5 rounded-full bg-zinc-500" />
                    <span className="text-muted-foreground text-xs">Code (excluded)</span>
                  </div>
                )}
              </div>
            ) : null}
        ...
        <div
          ...
          className={cn(
            "flex min-h-0 flex-col",
            isMobile ? (mobileTab === "results" ? "h-auto min-h-[350px] flex-1" : "hidden") : "h-full xl:flex-1"
          )}
        >
          <div className="bg-card relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl shadow-sm">
            <div className={cn(
              "relative min-h-0 flex-1 overflow-y-auto",
              isMobile ? "p-2 sm:p-4" : "p-4"
            )}>
    ...
    <Dialog open={loginDialogOpen} onOpenChange={setLoginDialogOpen}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            You need to be logged in to check for plagiarism. Please log in to
            continue.
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
