...
interface FormatRightPanelV2Props {
  className?: string;
}

type TabId = 'ai' | 'metadata' | 'checklist' | 'publish';
...
export function FormatRightPanelV2({ className }: FormatRightPanelV2Props) {
  ...
  const [metadata, setMetadata] = useState({
    ...
    series: 'Cosmic Chronicles',
    seriesNumber: '1',
    description: 'In a galaxy where stars are dying, one ship must carry the last hope of humanity across the void.',
    keywords: ['sci-fi', 'space', 'adventure', 'dystopian'],
    language: 'en',
  ...
  return (
    <aside className={cn(
      "w-[360px] border-l border-slate-200 dark:border-slate-800 flex flex-col bg-white dark:bg-[#101922]",
      className
    )}>
      {/* Header */}
          ...
          {activeTab === 'metadata' && (
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <div className="aspect-[2/3] bg-slate-100 dark:bg-slate-800 rounded-lg border-2 border-dashed border-slate-300 dark:border-slate-700 flex flex-col items-center justify-center cursor-pointer hover:border-[#137fec] transition-colors">
                  <Upload className="w-8 h-8 text-slate-400 mb-2" />
                  <span className="text-xs text-slate-500">Drop cover image here</span>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <input 
                    ...
                    className="w-full mt-1 px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:ring-1 focus:ring-[#137fec] focus:border-[#137fec]"
                  />
                </div>

                <div>
                  <label className="text-xs text-slate-600 dark:text-slate-400">Subtitle</label>
                  <input 
                    type="text" 
                    value={metadata.subtitle}
                    onChange={(e) => updateMetadata('subtitle', e.target.value)}
                    className="w-full mt-1 px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:ring-1 focus:ring-[#137fec]"
                  />
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs text-slate-600 dark:text-slate-400">Series</label>
                    <input 
                      type="text" 
                      value={metadata.series}
                      onChange={(e) => updateMetadata('series', e.target.value)}
                      className="w-full mt-1 px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm"
                    />
                  </div>
                  <div>
                    <label className="text-xs text-slate-600 dark:text-slate-400">Book #</label>
                    <input 
                      type="text" 
                      value={metadata.seriesNumber}
                      onChange={(e) => updateMetadata('seriesNumber', e.target.value)}
                      className="w-full mt-1 px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm"
                    />
                  </div>
                </div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  <label className="text-xs text-slate-600 dark:text-slate-400">Author Name *</label>
                  <input 
                    type="text" 
              ...
              <div className="space-y-3">
                ...
                <div>
                  <label className="text-xs text-slate-600 dark:text-slate-400">Book Description *</label>
                  <textarea 
                    rows={4}
                    value={metadata.description}
                    onChange={(e) => updateMetadata('description', e.target.value)}
                    className="w-full mt-1 px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm resize-none focus:ring-1 focus:ring-[#137fec]"
                  />
                  <div className="flex justify-between mt-1">
                    <span className="text-[10px] text-slate-400">Appears on retailer pages</span>
                    <span className="text-[10px] text-slate-400">{metadata.description.length}/4000</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs text-slate-600 dark:text-slate-400">Genre *</label>
                    <select 
                  ...
                  <div>
                    <label className="text-xs text-slate-600 dark:text-slate-400">Age Rating</label>
                    <select 
                      value={metadata.ageRating}
                      onChange={(e) => updateMetadata('ageRating', e.target.value)}
                      className="w-full mt-1 px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm"
                    >
                ...
                <div>
                  <label className="text-xs text-slate-600 dark:text-slate-400">Keywords (comma separated)</label>
                  <input 
                    type="text" 
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs text-slate-600 dark:text-slate-400">Publisher</label>
                    <input 
                      type="text" 
                  ...
                  <div>
                    <label className="text-xs text-slate-600 dark:text-slate-400">Publication Date</label>
                    <input 
                      type="date" 
                ...
                <div>
                  ...
                  <input 
                    ...
                    className="w-full mt-1 px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-mono"
