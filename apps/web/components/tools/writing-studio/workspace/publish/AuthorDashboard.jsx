...
function BookCard({ book, onSelect }) {
  ...
  return (
    <motion.div
      ...
      className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden hover:shadow-lg hover:border-[#137fec]/30 transition-all cursor-pointer group"
    ...
    >
      <div className="flex gap-4 p-4">
        {(book.coverUrl) ? (
          ...
          <div className="w-16 h-24 bg-gradient-to-br from-slate-200 to-slate-300 dark:from-slate-700 dark:to-slate-800 rounded-lg flex items-center justify-center shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <h4 className="font-bold text-sm text-slate-900 dark:text-white truncate group-hover:text-[#137fec] transition-colors">
                {book.title}
              </h4>
              {book.subtitle && (
        ...
        <ArrowRight className="h-4 w-4 text-slate-300 group-hover:text-[#137fec] transition-colors shrink-0 self-center" />
      </div>
    </motion.div>
  );
}
    ...
    >
      <div className="text-center max-w-md">
        <motion.div
          ...
          className="size-24 bg-[#137fec]/10 text-[#137fec] rounded-full flex items-center justify-center mx-auto mb-6"
        >
        ...
        <div className="grid grid-cols-3 gap-3 mb-8">
          {[
            ...
            <div
              key={label}
              className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl p-3"
            >
              <Icon className="h-5 w-5 text-[#137fec] mx-auto mb-1.5" />
        ...
        <button
          ...
          className="inline-flex items-center gap-2 px-8 py-3 bg-[#137fec] text-white rounded-xl font-bold shadow-lg shadow-[#137fec]/30 hover:bg-[#137fec]/90 transition-all text-sm"
        >
    ...
    <div className="flex-1 overflow-y-auto custom-scrollbar">
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-black text-slate-900 dark:text-white">My Books</h2>
            <p className="text-sm text-slate-500">Manage your published and upcoming books</p>
          </div>
          <button
            onClick={onStartPublishing}
            className="flex items-center gap-2 px-5 py-2.5 bg-[#137fec] text-white rounded-xl text-sm font-bold shadow-lg shadow-[#137fec]/20 hover:bg-[#137fec]/90 transition-all"
          >
            <Plus className="h-4 w-4" />
            New Book
          </button>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard
            ...
            label="Total Books"
            value={stats.total}
            color="bg-[#137fec]/10 text-[#137fec]"
          />
          <StatCard
            icon={Globe}
            label="Published"
            ...
            return (
              ...
              >
                ...
                <span className={cn(
                  ...
                  filter === f.id ? "bg-white/20" : "bg-slate-200 dark:bg-slate-700"
                )}>
                  {count}
                </span>
              </button>
        ...
        <div className="space-y-3">
          ...
          {filteredBooks.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-slate-500">No books found with this filter</p>
            </div>
          )}
        </div>
      </div>
