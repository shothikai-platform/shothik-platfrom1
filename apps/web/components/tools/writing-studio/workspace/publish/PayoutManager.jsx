...
const STATUS_CONFIG = {
  ...
  cancelled: { label: "Cancelled", color: "text-slate-600 bg-slate-100 dark:bg-slate-800", icon: XCircle },
};
...
function PayoutRequestForm({ availableBalance, onSubmit, onCancel, isRequesting, error }) {
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-slate-900/50 border-2 border-emerald-200 dark:border-emerald-800/30 rounded-xl p-5 space-y-4">
        ...
        <div>
          <label className="text-xs text-slate-500 block mb-1.5">Amount (min $25.00)</label>
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-7 pr-3 py-2.5 text-sm border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 font-mono"
            />
          </div>
        ...
        <div>
          <label className="text-xs text-slate-500 block mb-1.5">Payout Method</label>
          <div className="grid grid-cols-1 gap-2">
            {PAYOUT_METHODS.map((pm) => (
              ...
              >
                <pm.icon className={cn("h-5 w-5", method === pm.id ? "text-emerald-600" : "text-slate-400")} />
                <div className="flex-1 min-w-0">
                  <p className={cn("text-sm font-bold", method === pm.id ? "text-emerald-700 dark:text-emerald-400" : "text-slate-700 dark:text-slate-300")}>
                    {pm.name}
                  </p>
                  <p className="text-xs text-slate-500 truncate">{pm.description}</p>
                </div>
  ...
  return (
    <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl p-5">
      <div className="flex items-center gap-3 mb-3">
        ...
        <div>
          <h3 className="font-bold text-sm text-slate-900 dark:text-white">Set Up Stripe Connect</h3>
          <p className="text-xs text-slate-500">Connect your bank account to receive payouts</p>
        </div>
      </div>
      {error && (
      ...
      <button
        ...
        disabled={isLoading}
        className="w-full py-2.5 bg-[#137fec] hover:bg-[#1171d3] text-white rounded-xl text-sm font-bold transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
      >
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-16">
        <Loader2 className="h-8 w-8 text-[#137fec] animate-spin" />
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl p-4">
          ...
          <p className="text-2xl font-black text-slate-900 dark:text-white">${summary.pendingPayouts.toFixed(2)}</p>
        </div>
        <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
            <CheckCircle2 className="h-4 w-4 text-blue-500" />
            <span className="text-xs text-slate-500">Total Paid Out</span>
          </div>
          <p className="text-2xl font-black text-slate-900 dark:text-white">${summary.totalPaidOut.toFixed(2)}</p>
        </div>
      </div>
        ...
        {accounts.length === 0 ? (
          ...
          <div className="space-y-2">
            {accounts.map((account) => (
              <div key={account._id} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
                <CreditCard className="h-5 w-5 text-[#137fec]" />
                <div className="flex-1 min-w-0">
      ...
      <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl p-5">
        <h3 className="font-bold text-sm text-slate-700 dark:text-slate-300 mb-4 flex items-center gap-2">
          <Clock className="h-4 w-4 text-[#137fec]" />
          Payout History
        </h3>

        {history.length === 0 ? (
          <div className="text-center py-8">
            <DollarSign className="h-10 w-10 text-slate-200 dark:text-slate-700 mx-auto mb-2" />
          ...
          <div className="space-y-2">
            {history.map((payout) => {
              ...
              return (
                <div key={payout._id} className="flex items-center gap-3 p-3 border border-slate-100 dark:border-slate-800 rounded-lg">
