    ...
    <div className="space-y-6">
      <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <button className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-[#137fec] transition-colors">
                  ...
                  <div className="absolute left-5 top-10 bottom-0 w-0.5">
                    <div
                      className={cn(
                        ...
                        status.state === "completed" ? "bg-emerald-400" : "bg-slate-200 dark:bg-slate-700"
                      )}
                    />
                  </div>
                )}
                ...
                <div className="relative z-10">
                  <div
                    className={cn(
                      ...
                      status.state === "pending" && "bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-300 dark:text-slate-600"
                    )}
                  >
                ...
                <div className={cn("flex-1 pb-8", isLast && "pb-0")}>
                  <div className="flex items-center gap-2">
                    <p
                      className={cn(
                        ...
                        status.state === "active" && "text-slate-900 dark:text-white",
                        status.state === "completed" && "text-emerald-600 dark:text-emerald-400",
                        status.state === "pending" && "text-slate-400"
                      )}
                    >
                      {status.label}
                    </p>
                    {status.state === "active" && (
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#137fec]/10 text-[#137fec] font-bold animate-pulse">
                        Current
                      </span>
                    )}
                  </div>
                  ...
                  {status.id === "published" && status.state === "completed" && book?.googlePlayUrl && (
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 mt-2 text-xs text-[#137fec] font-bold hover:underline"
                    >
        ...
        >
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <button
                onClick={onResubmit}
                className="flex items-center gap-2 px-5 py-2.5 bg-[#137fec] hover:bg-[#137fec]/90 text-white rounded-xl text-sm font-bold transition-colors shadow-lg shadow-[#137fec]/20"
              >
                Edit & Resubmit
      ...
      {book?.reviewedBy && (
        <div className="bg-slate-50 dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700 rounded-xl p-4 flex items-start gap-3">
          <Search className="h-5 w-5 text-slate-400 shrink-0 mt-0.5" />
          <div>
