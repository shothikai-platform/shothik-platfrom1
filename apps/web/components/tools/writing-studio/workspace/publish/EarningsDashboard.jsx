...
function BookEarningsTable({ data }) {
  if (!data || data.length === 0) {
    return (
      <div className="text-center py-8">
        <BookOpen className="h-10 w-10 text-slate-200 dark:text-slate-700 mx-auto mb-2" />
        <p className="text-sm text-slate-400">No book earnings yet</p>
      </div>
    );
  }
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-200 dark:border-slate-700">
            <th className="text-left py-2 px-3 text-xs font-bold text-slate-500 uppercase">Book</th>
            <th className="text-right py-2 px-3 text-xs font-bold text-slate-500 uppercase">Units Sold</th>
            <th className="text-right py-2 px-3 text-xs font-bold text-slate-500 uppercase">Revenue</th>
            <th className="text-right py-2 px-3 text-xs font-bold text-slate-500 uppercase">Your Royalties</th>
          </tr>
        </thead>
        <tbody>
          {data.map((book) => (
            <tr key={book.bookId} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50">
        ...
        <tfoot>
          <tr className="border-t-2 border-slate-300 dark:border-slate-600">
            <td className="py-3 px-3 font-black text-slate-900 dark:text-white">Total</td>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl p-5">
        <h3 className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-4 flex items-center gap-2">
          <BarChart3 className="h-4 w-4 text-[#137fec]" />
          Monthly Royalties
        </h3>
      ...
      <div className="bg-slate-50 dark:bg-slate-900/30 rounded-xl p-4 border border-slate-200 dark:border-slate-800">
        <h4 className="text-xs font-bold text-slate-500 uppercase mb-2">How Royalties Work</h4>
