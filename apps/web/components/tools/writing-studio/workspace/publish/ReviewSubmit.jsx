...
function SectionCard({ icon: Icon, title, status, children, onEdit }) {
  return (
    <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
      <div className="flex items-center justify-between px-5 py-3 bg-slate-50 dark:bg-slate-800/30 border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center gap-2">
          <Icon className="h-4 w-4 text-[#137fec]" />
        ...
        <div className="flex items-center gap-2">
          ...
          {onEdit && (
            <button
              onClick={onEdit}
              className="text-xs text-[#137fec] font-bold hover:underline flex items-center gap-1"
            >
...
function DetailRow({ label, value, highlight }) {
  return (
    <div className="flex justify-between items-center py-1.5">
      ...
      <span className={cn("text-xs font-bold", highlight ? "text-emerald-600 dark:text-emerald-400" : "text-slate-900 dark:text-white")}>
        {value}
      </span>
    </div>
  );
      ...
      <div className={cn(
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            {isComplete && (
              <div className="flex items-start gap-2 text-[10px] text-slate-500 bg-white/60 dark:bg-slate-900/30 rounded-lg p-3">
                <Shield className="h-3.5 w-3.5 text-[#137fec] mt-0.5 shrink-0" />
                <span>
