  ...
  return (
    <div className="flex-1 overflow-y-auto custom-scrollbar">
      <div className="max-w-2xl mx-auto px-6 py-16 text-center">
        ...
        <div className="bg-white dark:bg-slate-900/50 border-2 border-purple-200 dark:border-purple-800/30 rounded-2xl p-6 mb-8 max-w-sm mx-auto text-left">
          <div className="flex items-center gap-2 mb-4">
            <Rocket className="h-5 w-5 text-purple-600" />
            <h3 className="font-bold text-lg text-slate-900 dark:text-white">Enterprise Plan</h3>
          </div>
          <div className="mb-4">
...
export function PublishView({ bookTitle = "The Midnight Protocol", project }) {
  ...
  const { books, isLoading } = useAuthorBooks(userId);
              ...
              <AnimatePresence mode="wait">
                ...
                {dashboardTab === "earnings" && (
                  <motion.div key="earnings" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <div className="max-w-4xl mx-auto px-6 py-6">
                      <h2 className="text-xl font-black text-slate-900 dark:text-white mb-1">Royalty Earnings</h2>
                      <p className="text-sm text-slate-500 mb-6">Track your book sales and royalties across all channels</p>
                ...
                {dashboardTab === "payouts" && (
                  <motion.div key="payouts" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <div className="max-w-4xl mx-auto px-6 py-6">
                      <h2 className="text-xl font-black text-slate-900 dark:text-white mb-1">Payouts</h2>
                      <p className="text-sm text-slate-500 mb-6">Manage your payout methods and request withdrawals</p>
                      <PayoutManager />
                    </div>
                  </motion.div>
                )}
                ...
                {dashboardTab === "distribution" && (
                  <motion.div key="distribution" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <div className="max-w-4xl mx-auto px-6 py-6">
                      <h2 className="text-xl font-black text-slate-900 dark:text-white mb-1">Distribution</h2>
                      <p className="text-sm text-slate-500 mb-6">Manage where your books are distributed worldwide</p>
          ...
          >
            <div className="px-6 pt-6 pb-2">
              <button
                ...
                className="flex items-center gap-2 text-sm text-slate-500 hover:text-[#137fec] transition-colors font-medium"
              >
        ...
        {view === "tracker" && selectedBook && (
          ...
          >
            <div className="max-w-3xl mx-auto px-6 py-8">
              <button
                onClick={handleBackToDashboard}
                className="flex items-center gap-2 text-sm text-slate-500 hover:text-[#137fec] transition-colors font-medium mb-6"
              >
                <ArrowLeft className="h-4 w-4" />
                Back to My Books
              </button>
              ...
              <div className="mb-6">
                ...
                {selectedBook.subtitle && (
                  <p className="text-slate-500 mt-1">{selectedBook.subtitle}</p>
                )}
              </div>
