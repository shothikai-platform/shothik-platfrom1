...
import { createProject, getTemplates } from "@/lib/projects-store";
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-white dark:bg-[#101922] rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden"
      ...
      >
        ...
        <div className="flex items-center gap-2 px-6 pt-4">
          {[1, 2, 3].map((s) => (
            <div key={s} className={`h-1 flex-1 rounded-full transition-all ${s <= step ? "bg-[#137fec]" : "bg-slate-200 dark:bg-slate-800"}`} />
          ))}
        </div>
        ...
        <div className="p-6 overflow-y-auto max-h-[60vh]">
          {step === 1 && (
            <div className="space-y-3">
              <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">What would you like to work on?</p>
                ...
                return (
                  <motion.button
                    ...
                    className={`w-full text-left p-5 rounded-xl border-2 transition-all ${
                      isSelected
                        ? "border-[#137fec] bg-blue-50/50 dark:bg-blue-950/20"
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-1.5 mt-3">
                          {card.features.map((f) => (
                            <span key={f} className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400">
                              {f}
                            </span>
                          ))}
                        </div>
                ...
                {templates.map((tmpl) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">{tmpl.description}</p>
                    </motion.button>
                  );
                })}
              </div>
          ...
          {step === 3 && (
            <div className="space-y-5">
              <div className="flex items-center gap-3 p-4 rounded-xl bg-gradient-to-r from-blue-50 to-transparent dark:from-blue-950/20 dark:to-transparent border border-blue-100 dark:border-blue-900/30">
                ...
                <div>
                  <p className="text-xs text-slate-500 dark:text-slate-400">{purposeConfig?.title}</p>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 text-sm bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-[#137fec] focus:ring-2 focus:ring-[#137fec]/10 transition-all text-slate-900 dark:text-white placeholder:text-slate-400"
                  autoFocus
              ...
              <div className="flex items-center gap-2 p-3 rounded-lg bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/30">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
          ...
          {step < 3 ? (
            <motion.button
              ...
              className="flex items-center gap-2 px-5 py-2.5 bg-[#137fec] hover:bg-[#1171d4] disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-xl text-sm font-semibold transition-colors"
            >
