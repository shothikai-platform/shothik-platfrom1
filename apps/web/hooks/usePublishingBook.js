...
export function usePublishingBook({ userId, bookId: existingBookId, initialTitle = "" }) {
  ...
  const ensureBookDraft = useCallback(async () => {
    ...
    setBookId(newId);
    return newId;
  }, [bookId, userId, initialTitle, createDraft]);
  ...
  const saveDraft = useCallback(
    async (updates) => {
      ...
      try {
        ...
        await updateDraft({ id, userId: userId || undefined, ...updates });
      } catch (err) {
    ...
    [ensureBookDraft, updateDraft, userId]
  );
  ...
  const uploadManuscript = useCallback(
    ...
    [ensureBookDraft, generateUploadUrl, saveManuscriptFile, userId]
  );
  ...
  const uploadCover = useCallback(
    ...
    [ensureBookDraft, generateUploadUrl, saveCoverFile, userId]
  );
  ...
  const submit = useCallback(async () => {
    ...
    setSaveError(null);
    try {
      await submitForReview({ bookId, userId: userId || undefined });
      return true;
    } catch (err) {
      setSaveError(err.message || "Submission failed");
      throw err;
    }
  }, [bookId, submitForReview, userId]);

  const resubmit = useCallback(async () => {
    if (!bookId) throw new Error("No book to resubmit");
    setSaveError(null);
    try {
      await resubmitForReviewMut({ bookId, userId: userId || undefined });
      return true;
    } catch (err) {
      setSaveError(err.message || "Resubmission failed");
      throw err;
    }
  }, [bookId, resubmitForReviewMut, userId]);
...
export function useAuthorBooks(userId) {
  const books = useQuery(api.books.listByUser, userId ? { userId } : "skip");
  return {
    books: books || [],
